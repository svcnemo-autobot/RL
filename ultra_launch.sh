...
# =============================================================================
# NOTE: do NOT realpath here. On this cluster /lustre/fsw/.../users/<u> is a symlink
# to /lustre/fs1/...; realpath rewrites paths to fs1, which the GB200 compute nodes
# do NOT mount (only /lustre/fsw). Keep the fsw literal so pyxis --container-workdir
# and mounts resolve on the workers.
...
# Tolerant: nemo-rl.env only exists in the custom-vLLM build. On the standard nightly it is
# absent, so skip it instead of failing line 1 (it only sets VLLM_GIT_REF/WHEEL_LOCATION).
...
# Redacted copy for printing/provenance so HF_TOKEN is never written to logs on shared FS.
if [[ -n "${HF_TOKEN:-}" ]]; then
