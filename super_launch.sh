...
export COMMAND="date ; \
    ...
    NEMO_GYM_SWE_AGENTS_DIR=${SNAPSHOT_DIR}/3rdparty/Gym-workspace/Gym/responses_api_agents/swe_agents \
...
# Only mount Megatron-LM if the source directory exists (branch may use Megatron-Bridge instead)
if [[ -d "${CODE_DIR}/3rdparty/Megatron-LM-workspace/Megatron-LM" ]]; then
...
SBATCH_CMD=(
    ...
    --exclude=pool0-00074
    --comment='{"OccupiedIdleGPUsJobReaper":{"exemptIdleTimeMins":"60","reason":"data_loading","description":"Async GRPO SWE training: training GPUs idle during rollout collection and validation each step"}}'
    ray.sub
)
